        ...
        <template #container>
            <div class="ios-modal w-[96vw] max-w-[1380px]" :style="modalStyle">
                <div class="ios-nav-bar cursor-grab active:cursor-grabbing select-none" @pointerdown="onDragStart">
                    ...
                    <span class="ios-nav-title">Personal DTR</span>
                ...
                <div class="ios-body max-h-[82vh] overflow-y-auto">
                    <div v-if="loadingSetup" class="ios-section pb-4">
                        <div class="ios-card p-8 text-center text-surface-400">
                            ...
                            <p class="text-sm">Loading personal DTR workspace…</p>
                        </div>
                    </div>

                    <DtrWorkspace v-else module-type="personal" :subject="subject" :reports="reports"
...
async function fetchSetup() {
    ...
    try {
        const { data } = await axios.get('/api/dtr/personal');
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: error.response?.data?.message ?? 'Could not load personal DTR workspace.',
            life: 4000,
        });
    } finally {
...
async function handleSaveReport(payload) {
    if (!canManageDtr.value) return;

    savingReport.value = true;

    try {
        await axios.post('/api/dtr/personal/reports', payload);
        await fetchSetup();

        toast.add({ severity: 'success', summary: 'Generated', detail: 'Personal DTR generated.', life: 3000 });
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: extractErrorMessage(error, 'Could not generate personal DTR.'),
            life: 4000,
        });
    } finally {
...
async function handleUpdateReport(payload, callbacks = {}) {
    ...
    try {
        ...
        await axios.put(`/api/dtr/personal/reports/${reportId}`, reportPayload);
        await fetchSetup();

        toast.add({ severity: 'success', summary: 'Updated', detail: 'Personal DTR updated.', life: 3000 });
        callbacks.onSuccess?.();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: extractErrorMessage(error, 'Could not update personal DTR.'),
            life: 4000,
        });
...
async function handleDeleteReport(payload, callbacks = {}) {
    ...
    try {
        ...
        await axios.delete(`/api/dtr/personal/reports/${reportId}`);
        await fetchSetup();

        toast.add({ severity: 'success', summary: 'Deleted', detail: 'Personal DTR deleted.', life: 3000 });
        callbacks.onSuccess?.();
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: extractErrorMessage(error, 'Could not delete personal DTR.'),
            life: 4000,
        });
        callbacks.onError?.(error);
    } finally {
