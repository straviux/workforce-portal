        ...
        <Dialog v-model:visible="showGenerateDialog" modal :header="generateDialogTitle" :draggable="false"
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-4">
                    <div class="ios-form-group xs:w-full w-1/4">
                ...
                <div v-if="!draftRows.length"
                    ...
                    Select the schedule range, then use the primary action below to prepare editable daily values.
                </div>
                ...
                <div v-if="draftRows.length" class="ios-card p-4">
                            ...
                            <thead>
                                <tr class="bg-surface-50 dark:bg-surface-900/60">
                                    ...
                                    <th v-for="task in draftRows" :key="task.sort_order"
                                        class="min-w-[180px] border border-surface-200 dark:border-surface-700 px-3 py-2 text-left align-top">
                                        <p class="font-semibold text-surface-800 dark:text-surface-100">
                                            Task {{ task.sort_order }}
                                        </p>
                                        <p
                                            class="mt-1 text-surface-600 dark:text-surface-300 whitespace-normal break-words">
                                            {{ task.task_name }}
                                        </p>
                                ...
                                <tr v-for="editorRow in draftEditorRows" :key="editorRow.work_date">
                                    ...
                                    <td v-for="taskValue in editorRow.task_values"
                                        ...
                                        <input v-if="taskValue.task_type === 'countable'"
                                            ...
                                            class="w-1/4 justify-center rounded-xl border border-surface-300 bg-white px-3 py-2 text-right text-sm text-surface-800 outline-none transition focus:border-sky-500 dark:border-surface-700 dark:bg-surface-950 dark:text-surface-100 cursor-pointer"
                                            :disabled="!canManage || isSavingReport" />

                                        <button v-else type="button"
                                            class="w-1/4 justify-center rounded-xl border px-3 py-2 text-sm font-semibold transition"
                    ...
                    <div v-else
                        ...
                        days, then rebuild the daily values.
                    </div>
                </div>
            </div>
...
const canPrepareDraft = computed(() => props.canManage
    ...
    && Boolean(selectedOfficeHead.value)
...
const canRunGenerateAction = computed(() => {
    ...
    return hasDraftPreview.value ? true : canPrepareDraft.value;
});
...
const generateActionLabel = computed(() => {
    if (!hasDraftPreview.value) return isEditingReport.value ? 'Rebuild Daily Values' : 'Prepare Daily Values';

    return isEditingReport.value ? 'Update SWA' : 'Save SWA';
});
const generateActionIcon = computed(() => {
    if (!hasDraftPreview.value) return isEditingReport.value ? 'pi pi-refresh' : 'pi pi-table';
...
const generateDialogHint = computed(() => {
    ...
    if (!hasValidCountableRanges.value) {
    ...
    if (hasDraftPreview.value) {
        return isEditingReport.value
            ? 'The saved daily values are editable below. Update when everything looks correct.'
            : 'The generated daily values are editable below. Save when everything looks correct.';
    }

    return isEditingReport.value
        ? 'Rebuild the daily values table after changing the schedule, then update the record.'
